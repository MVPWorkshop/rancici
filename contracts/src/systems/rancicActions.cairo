...
mod rancicActions {
    use dojo_starter::models::{
        rancic::{ERC721Meta, ERC721OperatorApproval, ERC721Owner, ERC721Balance, ERC721TokenApproval}
    };
    use dojo_starter::interfaces::rancic;
    ...
    fn constructor(
        ...
        self.initializer(name, symbol, base_uri, total_space); // add maybe randomnes on total_space
        self._mint(recipient, token_id);
    }

    #[abi(embed_v0)]
    impl ERC721MetadataImpl of rancic::IERC721Metadata<ContractState> {
        ...
        fn total_space(self: @ContractState, token_id: u256) -> u32 {
            assert(self._exists(token_id), Errors::INVALID_TOKEN_ID);
            // TODO : concat with id
            self.get_total_space(token_id)
        }
    }

    #[abi(embed_v0)]
    impl ERC721MetadataCamelOnlyImpl of rancic::IERC721MetadataCamelOnly<ContractState> {
        fn tokenURI(self: @ContractState, tokenId: u256) -> felt252 {
            assert(self._exists(tokenId), Errors::INVALID_TOKEN_ID);
            self.get_uri(tokenId)
        }

        fn totalSpace(self: @ContractState, tokenId: u256) -> u32 {
            assert(self._exists(tokenId), Errors::INVALID_TOKEN_ID);
            self.get_total_space(tokenId)
        }

    }

    #[abi(embed_v0)]
    impl ERC721Impl of rancic::IERC721<ContractState> {
    ...
    #[abi(embed_v0)]
    impl ERC721CamelOnlyImpl of rancic::IERC721CamelOnly<ContractState> {
    ...
    impl WorldInteractionsImpl of WorldInteractionsTrait {
        ...
        fn get_total_space(self: @ContractState, token_id: u256) -> u32 {
            self.get_meta().total_space
        }
    ...
    impl InternalImpl of InternalTrait {
        fn initializer(ref self: ContractState, name: felt252, symbol: felt252, base_uri: felt252, total_space: u32) {
            // TODO: add randomnes on total_space when initializing
            let meta = ERC721Meta { token: get_contract_address(), name, symbol, base_uri, total_space };
            set!(self.world(), (meta));
        }
